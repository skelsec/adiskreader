...
from adiskreader.filesystems.fat.DosDateTime import DosDateTime
from adiskreader.filesystems.fat.EightDotThree import EightDotThree
from adiskreader.filesystems.fat._exceptions import PyFATException, NotAnLFNEntryException,\
    BrokenLFNEntryException
from adiskreader.filesystems.fat import FAT_OEM_ENCODING, FAT_LFN_ENCODING
...
class FATDirectoryEntry:
    ...
    async def add_subdirectory(self, dir_entry, recursive: bool = True):
    ...
    async def __populate_dirs(self):
        if self.__lazy_load is False:
            return

        clus = self.get_cluster()
        async for dir_entry in self.__fs.parse_dir_entries_in_cluster_chain(clus):
    ...
    async def _get_entries_raw(self):
    ...
    async def get_entries(self):
        ...
        entries = await self._get_entries_raw()
        for d in entries:
    ...
    async def _search_entry(self, name: str):
    ...
    async def get_entry(self, path: str):
        ...
        for segment in filter(None, path.split("\\")):
            ...
            entry = await entry._search_entry(segment)
        return entry
    ...
    async def walk(self):
        ...
        for d in self.__dirs:
            ...
            async for root, dirs, files in d.walk():
                yield root, dirs, files
